...
public class MenuControl {
	...
	public void detailOfMenu(HttpServletRequest request,
		...
		if (id != null && id.length() > 0) {
			...
			if (Integer.parseInt(num) == 1)
				request.getRequestDispatcher("houtai/pages/menu/detail.jsp")
						.forward(request, response);
			if (Integer.parseInt(num) == 2)
				request.getRequestDispatcher("houtai/pages/menu/edit.jsp")
						.forward(request, response);
		}
	...
	public void checkNameOfMenu(HttpServletRequest request,
		...
		if (name != null && name.length() > 0) {
			...
			Fmenu fm=ms.selectFmenuByName(name);
			if (m3 != null&&fm!=null) {
				response.setContentType("text/html;charset=utf-8");
				PrintWriter out = response.getWriter();
				out.print("fail");
	...
	public void selectAllRoles(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		//查询父菜单
		List<Fmenu> fms = ms.selecFmenus();
		//查询子菜单
		List<Menu> mes = ms.selectAllMenus();
		request.setAttribute("menus", fms);
