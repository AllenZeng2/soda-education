...
<script type="text/javascript">
	function tijiao(num) {
		...
		window.location.href = "ZaixianServlet?page=" + num;
				...
				<table class="buyTable" cellpadding="0" cellspacing="0"
					...
					<c:forEach items="${course }" var="course">
						<tr>
							...
							<td><a href="CourseInfoServlet?param=2&ce_id=${course.ce_id }">${course.ce_name }</a></td>
							<td>${course.user.ur_name }</td>
							<td>${course.ce_money }</td>
							<td><p>${course.ce_easy }</p>
							</td>
						</tr>
					</c:forEach>
